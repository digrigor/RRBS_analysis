...
p1_suffix = "R1_001.fastq.gz"
p2_suffix = "R2_001.fastq.gz"
...
parallel_command(command_tgalore, 20, trimgalore_result_dir, log)
...
##PE Meth Alignment - BISMARK PE directional
log = "bismark_PE_meth_log.txt"
suffix1 = "val_1.fq.gz"
suffix2 = "val_2.fq.gz"
aligning_command_pe = get_bismark_commands_pe(bismark_dir, bowtie2_dir, meth_ref, samtools_dir, trimgalore_result_dir, temp_dir, suffix1, suffix2, aligned_pe_dir, status='meth')
parallel_command(aligning_command_pe, 4, aligned_pe_dir, log)

##PE unmeth Alignment - BISMARK PE directional
log = "bismark_PE_unmeth_log.txt"
suffix1 = "val_1.fq.gz"
suffix2 = "val_2.fq.gz"
aligning_command_pe = get_bismark_commands_pe(bismark_dir, bowtie2_dir, unmeth_ref, samtools_dir, trimgalore_result_dir, temp_dir, suffix1, suffix2, aligned_pe_dir, status='unmeth')
parallel_command(aligning_command_pe, 4, aligned_pe_dir, log)
...
##Merging lanes (bam files) for meth PE
log = "merge_aligned_meth_PE_log.txt"
merging_lanes_pe_command = get_merging_bam_lanes_command(samtools_func, aligned_pe_dir, alinged_merged_pe_dir, unique_sample_names, read='', status='meth', mode='pe')
parallel_command(merging_lanes_pe_command, 20, alinged_merged_pe_dir, log)

##Merging lanes (bam files) for unmeth PE
log = "merge_aligned_unmeth_PE_log.txt"
merging_lanes_pe_command = get_merging_bam_lanes_command(samtools_func, aligned_pe_dir, alinged_merged_pe_dir, unique_sample_names, read='', status='unmeth', mode='pe')
parallel_command(merging_lanes_pe_command, 20, alinged_merged_pe_dir, log)
...
##Methylation Extraction for sorted methylated bam files
log = "methcal_for_meth_pe_log.txt"
command_mextraction = get_mExtraction(bismark_methCall_func, samtools_dir, sorted_dir, bedgraph_cpg_dir, meth_ref, status='meth')
parallel_command(command_mextraction, 15, methcal_dir, log)
outlog+='Methylation Extraction: Done!\n'

##Methylation Extraction for sorted unmethylated bam files
log = "methcal_for_unmeth_pe_log.txt"
command_mextraction = get_mExtraction(bismark_methCall_func, samtools_dir, sorted_dir, bedgraph_cpg_dir, unmeth_ref, status='unmeth')
parallel_command(command_mextraction, 15, methcal_dir, log)
outlog+='Methylation Extraction: Done!\n'
